...
--- Generates table of hottest callers of a symbol (without known file location).
-- @param event Event to use.
-- @param symbol Symbol name.
-- @return tab, count where tab is a table of entries and count is the total event count.
function M.hottest_callers_symbol_table(event, symbol)
    local cg = callgraph.callgraphs[event]

    if not cg.node_info.symbol or not cg.node_info.symbol[symbol] then
        vim.notify("No callgraph information for symbol: " .. symbol, vim.log.levels.ERROR)
        return nil
    end

    local total_count = cg.node_info.symbol[symbol].rec_count
    local in_counts = cg.node_info.symbol[symbol].in_counts
    local entries = {}
...
--- Generates table of hottest callees of a symbol (without known file location).
-- @param event Event to use.
-- @param symbol Symbol name.
-- @return tab, count where tab is a table of entries and count is the total event count.
function M.hottest_callees_symbol_table(event, symbol)
    local cg = callgraph.callgraphs[event]
    ...
    local total_count = cg.node_info.symbol[symbol].rec_count
    local out_counts = cg.node_info.symbol[symbol].out_counts
    local entries = {}

    for out_file, file_tbl in pairs(out_counts) do
        for out_line, count in pairs(file_tbl) do
            if config.format(count, total_count) then
                table.insert(entries, entry_from_line(out_file, out_line, count))
            end
        end
    end
